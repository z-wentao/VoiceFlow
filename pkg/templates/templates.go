...
<div id="maimemo-form-%s" hidden>
	...
	<input type="text" id="token-%s" placeholder="输入 Token" onchange="saveToken(this.value)">
	<br>
	<label>云词本 ID:</label>
	<input type="text" id="notepad-%s" placeholder="输入云词本 ID" onchange="saveNotepadId(this.value)">
	<button hx-post="/api/maimemo/list-notepads"
	        hx-include="#token-%s"
	        hx-target="#notepad-list-%s"
	        hx-swap="innerHTML">🔍 查询云词本</button>
	<div id="notepad-list-%s" hidden></div>
	<br>
...
func RenderNotepads(notepads []map[string]interface{}, jobID string) template.HTML {
	if len(notepads) == 0 {
		return template.HTML("<p>没有云词本</p>")
	}
	...
	for _, notepad := range notepads {
		...
		html.WriteString(fmt.Sprintf(`
<li onclick="selectNotepad('%s', '%s')">%s (ID: %s)</li>
